      ...
      <div className='relative w-full py-12 min-h-[400px] overflow-x-scroll my-4 flex flex-col items-center justify-start p-4 border border-gray-300 rounded-md gap-3'>
        ...
        <div className='absolute top-4 left-4'>
          <p className='text-sm font-semibold'>
            Contador : <span className='text-xl font-bold text-textColor'>{allUsers?.length}</span>
          </p>
        </div>
...
export const DashboardUserCard = ({data, index}) => {
  ...
  return (
    ...
    >
      ...
      <p className='text-base text-textColor w-275 min-w-[160px] text-center'>{data.email_verified ? "Verificado" : "Falso"}</p>
      ...
      <p className='text-base text-textColor text-center'>{data.role === 'admin' ? 'Admin' : 'Miembro'}</p>
      ...
      {isUserRoleUpdated && (
        ...
        >
          <p className='text-textColor text-sm font-semibold'>¿Quieres marcar este usuario como <span>{data.role !== "admin" ? "Administrador" : "Miembro"}</span>?</p>

          <div className='flex items-center justify-center text-center gap-4'>
            <motion.button
              whileTap={{scale : 0.75}}
              className='outline-none border-none text-sm px-4 py-1 rounded-md bg-sky-400 text-black hover:shadow-md'
